...
app.get("/api/member/submissions", (req, res) => res.json(db.prepare("SELECT item_no, name, sss_no, account_type, account_number, status, rejection_reason, date_enrolled, date_reviewed, processing_days FROM records WHERE member_id IS NULL ORDER BY item_no DESC").all()));
app.post("/api/member/submissions", (req, res) => {
  ...
  db.prepare("INSERT INTO records (item_no, member_id, name, sss_no, account_type, account_number, status, rejection_reason, date_enrolled, date_reviewed, processing_days, enrollment_month, enrollment_year, duplicate_flag) VALUES (?, NULL, ?, ?, ?, ?, 'Submitted', NULL, ?, '', 0, ?, ?, NULL)").run(itemNo, clientName, clientSss, accountType, accountNumber, enrolled, month, new Date().getUTCFullYear());
  audit(`submitted record ${itemNo}`); res.status(201).json(db.prepare("SELECT * FROM records WHERE item_no = ?").get(itemNo));
});
app.patch("/api/member/submissions/:itemNo", requireMember, (req, res) => { const result = db.prepare("UPDATE records SET account_type = ?, account_number = ?, status = 'Submitted', rejection_reason = NULL WHERE item_no = ? AND member_id = ? AND status IN ('Rejected', 'Correction Required')").run(String(req.body.accountType || "").trim(), String(req.body.accountNumber || "").trim(), Number(req.params.itemNo), req.session.member.id); if (!result.changes) return res.status(404).json({ error: "Submission not found or cannot be corrected" }); audit(req.session.member, `corrected record ${req.params.itemNo}`); res.json(db.prepare("SELECT * FROM records WHERE item_no = ?").get(Number(req.params.itemNo))); });
