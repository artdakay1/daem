...
app.post("/api/member/register", (req, res) => {
  ...
  try { const result = db.prepare("INSERT INTO members (username, password_hash, name, sss_no, approval_status) VALUES (?, ?, ?, ?, 'Pending')").run(username, bcrypt.hashSync(password, 12), name, sssNo); res.status(201).json({ id: result.lastInsertRowid, username, name, sssNo, approvalStatus: "Pending" }); } catch (error) { res.status(409).json({ error: error.message.includes("UNIQUE") ? "Username is already registered" : "Unable to register member" }); }
});
app.post("/api/member/login", (req, res) => { const member = db.prepare("SELECT * FROM members WHERE username = ?").get(String(req.body.username || "")); if (!member || !bcrypt.compareSync(String(req.body.password || ""), member.password_hash)) return res.status(401).json({ error: "Invalid member credentials" }); if (member.approval_status !== "Approved") return res.status(403).json({ error: member.approval_status === "Rejected" ? "This member account was rejected by the administrator" : "Your account is awaiting administrator approval" }); req.session.member = { id: member.id, username: member.username, name: member.name, sssNo: member.sss_no, approvalStatus: member.approval_status }; res.json(req.session.member); });
...
app.listen(port, host, () => console.log(`DAEM member server listening on http://${host}:${port}`));
